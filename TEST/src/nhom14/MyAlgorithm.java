...
public class MyAlgorithm {
	...
	private	ArrayList<ArrayList <Pair>> arrLine = new ArrayList<ArrayList<Pair>>(); // mang danh sach ke
	private ArrayList<ArrayList <Integer>> arrsolution = new ArrayList<ArrayList <Integer>>();// mang duong di
	...
	public ArrayList<ArrayList<Pair>> getArrLine() {
		return arrLine;
	}

	public void setArrLine(ArrayList<ArrayList<Pair>> arrLine) {
		this.arrLine = arrLine;
	}

	public ArrayList<ArrayList<Integer>> getArrsolution() {
		return arrsolution;
	}

	public void setArrsolution(ArrayList<ArrayList<Integer>> arrsolution) {
		this.arrsolution = arrsolution;
	}
	...
	public void Try(int stepP) {
		if(stepP == endPoint){
			ArrayList<Integer> ss = new ArrayList<Integer>();
			for(int i = 0; i < solution.size();i++){
				ss.add(solution.get(i));
			}
			arrsolution.add(ss);
		}
		for(int i = 0; i < arrLine.get(stepP).size();i++){
			int kk = arrLine.get(stepP).get(i).getKey();
			if(flag[kk] == false){
				solution.add(kk);
				flag[kk] = true;
				Try(kk);
	...
	public void allsolution() {
		...
		int sizeline = arrMyLine.size();
		for(int i = 0; i < size; i++){
			arrLine.add(new ArrayList<Pair>());
		}
		...
		for(int i = 0; i < arrsolution.size(); i++){
			for(int j = 0; j < arrsolution.get(i).size();j++){
				if(j == arrsolution.get(i).size() - 1){
					System.out.print(arrsolution.get(i).get(j));
				} else {
					System.out.print(arrsolution.get(i).get(j) + "->");
				}
			}
